...
- **Mac:** macOS 13 o superior. · **Windows:** Windows 10 (1809) o superior, 64 bits. · **Linux:** distro con glibc (Ubuntu, Debian, Fedora, Arch…), 64 bits.
- Conexión a internet.
...
Cuando termine, busca **"Claude Terminal"** en tu **menú de aplicaciones** (y en el Escritorio, si
tienes), o escribe `claude-terminal` en una terminal nueva. ✅

> 🔒 **¿Es seguro?** Sí. El instalador es de código abierto y puedes leerlo antes de
> ejecutarlo aquí: **[`install-linux.sh`](./install-linux.sh)**. No instala nada oculto.

**Notas:**
- Funciona en **escritorio** (GNOME, KDE, XFCE…) y también en **servidores / WSL** sin entorno
  gráfico: en ese caso instala Claude igual y omite el ícono (usa `claude-terminal` o `claude`).
- El **selector gráfico de carpetas** (opción 4 del menú) usa `zenity` o `kdialog`. Si no los
  tienes, instala uno (`sudo apt install zenity`) o usa la opción 5 para escribir la ruta.

### O descarga el archivo

Descarga **[`install-linux.sh`](https://raw.githubusercontent.com/Hainrixz/claude-cmd/main/install-linux.sh)** y ejecútalo:
...
|---|---|---|
| `command not found: claude` / `'claude' no se reconoce` | El PATH no se recargó | **Cierra y abre** una terminal nueva. Mac: o ejecuta `source ~/.zshrc`; Linux: `source ~/.bashrc`. (El ícono/lanzador funciona igual.) |
...
| (Linux) "Claude Terminal" no aparece en el menú o el Escritorio | El menú de apps no se refrescó, o GNOME no confía en el lanzador | Cierra sesión y vuelve a entrar; o ejecuta `claude-terminal` en una terminal. En GNOME, clic derecho en el ícono del Escritorio → **"Permitir ejecución / Allow Launching"**. |
| (Linux) la opción 4 (elegir carpeta) no abre ninguna ventana | Falta `zenity` o `kdialog` | Instala uno: `sudo apt install zenity` (Debian/Ubuntu), o usa la **opción 5** para escribir/pegar la ruta. |
| (Linux) "cannot execute: required file not found" / errores de glibc | Distro con musl (Alpine) | Claude Code oficial necesita **glibc**. Usa una distro basada en glibc (Ubuntu, Debian, Fedora, Arch…). |
...
- **Linux:**
  ```bash
  rm -f ~/.local/bin/claude ~/.local/bin/claude-terminal && rm -rf ~/.local/share/claude
  rm -f ~/.local/share/applications/claude-terminal.desktop ~/Desktop/claude-terminal.desktop
  rm -f ~/.local/share/icons/claude-terminal.png
  ```
- **Windows:**
  ```powershell
...
(`https://claude.ai/install.sh` en Mac y Linux, `https://claude.ai/install.ps1` en Windows), que
instala Claude Code por-usuario, sin Node ni permisos de administrador. Encima de eso:
...
NSWorkspace, en Windows como `.ico` en `%LOCALAPPDATA%\claude-cmd`, y en Linux como `.png` referido
desde un archivo `.desktop`). Si el ícono no se pudiera aplicar, el lanzador funciona igual.
